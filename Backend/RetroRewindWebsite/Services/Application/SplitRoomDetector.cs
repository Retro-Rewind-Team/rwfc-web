...
public class SplitRoomDetector : ISplitRoomDetector
{
    ...
    private List<RoomDto> DetectSplitInRoom(RoomDto room)
    {
        ...
        foreach (var (index, component) in components.Select((c, i) => (i, c)))
        {
            var subRoom = new RoomDto
            {
                ...
                ConnectedPlayerIds = component.Select(p => p.Pid).ToList()
            };
        ...
        subRooms = subRooms.OrderBy(r => r.IsSplit).ToList();
    ...
    private Dictionary<string, HashSet<string>> BuildConnectionGraph(List<RoomPlayerDto> players)
    {
        ...
        for (int i = 0; i < players.Count; i++)
        {
            ...
            graph[player.Pid] = new HashSet<string>();
    ...
    private List<List<RoomPlayerDto>> FindConnectedComponents(
    ...
    private int? CalculateAverageVR(List<RoomPlayerDto> players)
    {
