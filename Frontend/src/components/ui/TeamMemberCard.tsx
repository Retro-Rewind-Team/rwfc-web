...
export default function TeamMemberCard(props: TeamMemberCardProps) {
    const shadowRgb = () => props.roleColor?.shadowColor || "156, 163, 175";
        ...
        <div class="w-full sm:w-[calc(50%-0.75rem)] lg:w-[calc(33.333%-1rem)] xl:w-[calc(25%-1.125rem)] group">
            ...
            >
                ...
                <div class="relative w-24 h-24 mx-auto mb-4">
                    <div
                        class={`absolute inset-0 rounded-full border-3 ${props.roleColor?.accent || "border-gray-300"} opacity-0 group-hover:opacity-100 transition-opacity`}
                    ></div>
                    <Show
                        when={props.member.image}
                        fallback={
                            <div
                                class={`w-full h-full rounded-full flex items-center justify-center text-white text-3xl font-bold shadow-sm ${props.roleColor?.accent || "bg-gray-400"} bg-current`}
                            >
